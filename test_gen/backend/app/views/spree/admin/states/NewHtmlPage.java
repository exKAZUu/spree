...
public class NewHtmlPage extends AbstractPage {
  ...
  @FindBy(how = How.XPATH, using = "//*[@id='__pogen_601']")
  private WebElement Spree_dot_t__new_state__;
  
  public WebElement getElementOfSpree_dot_t__new_state__() {
    return Spree_dot_t__new_state__;
  }
  
  public String getTextOfSpree_dot_t__new_state__() {
    Matcher matcher = commentPattern.matcher(driver.getPageSource());
    while (matcher.find()) {
      if (matcher.group(1).equals("__pogen_601") && matcher.group(2).equals("Spree_dot_t__new_state__")) {
  ...
  @FindBy(how = How.XPATH, using = "//*[@id='__pogen_600']")
  ...
  public String getTextOfRender__partial_____form____locals_______f____f___() {
    Matcher matcher = commentPattern.matcher(driver.getPageSource());
    while (matcher.find()) {
      if (matcher.group(1).equals("__pogen_600") && matcher.group(2).equals("render__partial_____form____locals_______f____f___")) {
        return matcher.group(3);
      }
    }
    return null;
  ...
  public String getTextOfRender__partial_____spree_divide_admin_divide_shared_divide_new_resource_links__() {
    Matcher matcher = commentPattern.matcher(driver.getPageSource());
    while (matcher.find()) {
      if (matcher.group(1).equals("__pogen_600") && matcher.group(2).equals("render__partial_____spree_divide_admin_divide_shared_divide_new_resource_links__")) {
        return matcher.group(3);
      }
    }
    return null;
